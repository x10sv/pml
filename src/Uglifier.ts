...
export default vscode.commands.registerCommand('extension.pmlUglify', () => {
    ...
    let lastLine = document.lineAt(lines - 1);
    let start = new vscode.Position(0, 0);
    let end = new vscode.Position(lastLine.lineNumber, lastLine.text.length);

    let variables: string[] = [];

    let newContent = '--<001>-- Published PML 1.0 SP1 >--\n';

    for (let l = 0; l < lines; l++) {
        let lineContent = document.lineAt(l).text;
        ...
        if (
            ...
            lineContent.indexOf('|') == -1 &&
            lineContent.indexOf('"') == -1 &&
            lineContent.indexOf('$P') == -1 &&
            lineContent.indexOf('$p') == -1 &&
            lineContent.indexOf('CE') == -1
        ) {
